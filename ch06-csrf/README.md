...
| attacker | http://localhost:8888/attacker.html | 공격자의 악성 웹서버 |
...
| SameSite 쿠키 | 설정 없음 (브라우저 기본값 Lax 적용) | `SameSite=Strict` (동일 사이트만 허용) |
...
- `attacker` (포트 8888) — 공격자 웹서버 (`attacker.html` 호스팅)
...
**같은 브라우저**의 새 탭에서 http://localhost:8888/attacker.html 접속

> **중요:** `attacker.html`을 로컬 파일(`file://`)로 열면 **쿠키가 전송되지 않아 공격이 실패**합니다.
> 반드시 `http://localhost:8888/attacker.html` 으로 접속해야 합니다.
...
> **참고:** localhost에서는 다른 포트(5001, 8888)도 **same-site**로 취급되므로,
> 브라우저 기본 SameSite=Lax 정책에서도 CSRF 공격이 동작합니다.
...
[2. alice가 공격자 페이지를 방문 (localhost:8888)]
    → "경품 수령하기" 버튼 클릭
    → 숨겨진 <form>이 localhost:5001/transfer로 POST 전송
    → localhost:8888 → localhost:5001은 same-site이므로 쿠키 자동 포함!
...
```python
# 취약: SameSite 설정 없음 (브라우저 기본값 Lax 적용)
# 참고: SameSite=None + Secure=False는 최신 브라우저에서 차단됨
# app.config['SESSION_COOKIE_SAMESITE'] = 'None'  # 주석 처리
...
2. 잔액 1,000원 확인
3. **동일 브라우저 새 탭**에서 http://localhost:8888/attacker.html 접속
...
| 문제 | 원인 | 해결 방법 |
|------|------|-----------|
| "Not logged in" 반환 | `file://`로 열면 쿠키 미전송 | `http://localhost:8888/attacker.html`로 접속 |
| 잔액이 변하지 않음 | Docker 재시작 시 DB 초기화 필요 | `docker-compose down && docker-compose up -d` |
| 포트 8888 충돌 | 다른 서비스가 8888 사용 중 | docker-compose.yml에서 attacker 포트 변경 (예: 9100:8888) |
